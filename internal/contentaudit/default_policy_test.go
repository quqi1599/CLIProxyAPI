...
func TestDefaultPolicyManagedCountsAndActions(t *testing.T) {
	...
	if decision := matcher.Match("虚构段落包含飞机杯、生殖器、乳头和阴茎等露骨描述"); !decision.Matched || decision.RuleID != "block-sexual-dense-explicit-content" || decision.Action != RuleActionObserve {
