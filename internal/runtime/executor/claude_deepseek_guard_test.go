...
func TestClaudeDeepSeekOfficialLongContextToolPileLimits(t *testing.T) {
	for _, model := range []string{"deepseek-v4-pro", "deepseek-v4-flash(1m)"} {
		...
		oldBoundaryBody := make([]byte, largeClaudeCompatToolResultPilePayloadBytes)
		oldBoundaryPreflight := claudeCompatPreflight{
			hasToolUse:             true,
			hasToolResult:          true,
			toolResultOnlyMessages: largeClaudeCompatToolResultOnlyMessages,
		}
		if reason, rejected := largeClaudeCompatToolHistoryRejectReason(oldBoundaryBody, meta, oldBoundaryPreflight); rejected {
			t.Fatalf("model %q rejected at legacy 3 MiB/120 boundary: %s", model, reason)
		}
	}
}
