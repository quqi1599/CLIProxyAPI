...
func TestNewCodexStatusErrAssignsFailureScope(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "credential quota exhausted",
			statusCode: http.StatusTooManyRequests,
			body:       []byte(`{"error":{"type":"quota_exhausted","code":"quota_exhausted","message":"account quota exhausted"}}`),
			wantKind:   failurecontract.QuotaExceeded,
			wantScope:  failurecontract.ScopeCredential,
			retryable:  true,
		},
		{
			name:       "credential quota exceeded",
			statusCode: http.StatusTooManyRequests,
			body:       []byte(`{"error":{"type":"quota_exceeded","code":"quota_exceeded","message":"account quota exceeded"}}`),
			wantKind:   failurecontract.QuotaExceeded,
			wantScope:  failurecontract.ScopeCredential,
			retryable:  true,
		},
