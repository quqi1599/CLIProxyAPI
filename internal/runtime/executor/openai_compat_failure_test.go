...
func TestNewOpenAICompatStatusErrTypedFailureParity(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "403 model permission is model scoped",
			status:    http.StatusForbidden,
			body:      `{"error":{"message":"model access denied","code":"permission_denied"}}`,
		...
		},
		{
			...
			status:    http.StatusTooManyRequests,
			body:      `{"error":{"message":"account quota exhausted","code":"quota_exhausted"}}`,
			wantKind:  failurecontract.QuotaExceeded,
			wantScope: failurecontract.ScopeCredential,
			wantRetry: true,
		},
		{
			...
			status:    http.StatusTooManyRequests,
			body:      `{"error":{"message":"account quota exceeded","code":"quota_exceeded"}}`,
			wantKind:  failurecontract.QuotaExceeded,
			wantScope: failurecontract.ScopeCredential,
			wantRetry: true,
		},
