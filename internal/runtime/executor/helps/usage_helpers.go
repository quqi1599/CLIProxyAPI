...
func parseClaudeUsageNode(usageNode gjson.Result) usage.Detail {
	...
	if reasoningTokens > 0 && reasoningTokens <= rawOutputTokens {
		...
		// avoid inventing extra non-reasoning output for inconsistent payloads.
	...
	// output_tokens already includes thinking; cache fields are independent from
	// input_tokens in the Messages API.
