...
func TestTranslateRequestGuardedRejectsLegacyTranslatorAmplification(t *testing.T) {
	...
	registry.Register(from, to, func(_ string, input []byte, _ bool) []byte {
	...
	for _, stream := range []bool{false, true} {
		ctx := sdktranslator.ContextWithRegistry(internalpayload.WithTransformReport(context.Background(), int64(len(input))), registry)
...
func TestTranslateRequestGuardedAllowsNormalLargeRequest(t *testing.T) {
	...
	output, errTranslate := TranslateRequestGuarded(ctx, "legacy.translate.test", from, to, "", input, false, internalpayload.AmplificationOverride{})
	if errTranslate != nil {
...
func TestTranslateRequestPairGuardedReusesIdenticalOutput(t *testing.T) {
	...
	ctx := sdktranslator.ContextWithRegistry(context.Background(), registry)
	original, active, errTranslate := TranslateRequestPairGuarded(ctx, "legacy.translate.test", from, to, "", input, append([]byte(nil), input...), false, internalpayload.AmplificationOverride{})
	if errTranslate != nil {
