	...
	tests := []struct {
	...
	}{
		{
			...
			run: func() error {
				...
				_, err := executor.prepareOpenAICompatRequest(context.Background(), nil, cliproxyexecutor.Request{Model: "qwen3.7-plus", Payload: body}, cliproxyexecutor.Options{SourceFormat: sdktranslator.FormatOpenAI}, "https://example.com/compatible-mode/v1", "qwen3.7-plus", openAICompatProfileForKind("qwen"), false)
				return err
			},
		},
		{
			...
			run: func() error {
				...
				auth := &cliproxyauth.Auth{Provider: "claude", Attributes: map[string]string{"api_key": "test-key", "base_url": "https://example.com/apps/anthropic", "compat_kind": "qwen"}}
				_, err := executor.prepareClaudeRequest(context.Background(), auth, cliproxyexecutor.Request{Model: "qwen3.7-plus", Payload: body}, cliproxyexecutor.Options{SourceFormat: sdktranslator.FormatClaude}, "qwen3.7-plus", false)
				return err
			},
		},
	}
