...
func newResponsesWebsocketToolCacheTurn(outputCache, callCache *websocketToolOutputCache, sessionKey string) *responsesWebsocketToolCacheTurn {
	sessionKey = normalizeResponsesWebsocketSessionKey(sessionKey)
	if sessionKey == "" || outputCache == nil {
		return nil
	}
...
func (t *responsesWebsocketToolCacheTurn) recordResponse(payload []byte) {
	...
	case "response.output_item.added", "response.output_item.done":
		t.recordItem(gjson.GetBytes(payload, "item"))
	}
}
...
func (t *responsesWebsocketToolCacheTurn) commit() {
	...
	if t.outputCache != nil {
		for _, callID := range t.outputOrder {
			t.outputCache.record(t.sessionKey, callID, t.outputs[callID])
		}
	}
	if t.callCache != nil {
		for _, callID := range t.callOrder {
			t.callCache.record(t.sessionKey, callID, t.calls[callID])
		}
	}
}

func repairResponsesWebsocketToolCallsWithoutRecording(outputCache, callCache *websocketToolOutputCache, sessionKey string, payload []byte) []byte {
	return repairResponsesWebsocketToolCallsWithCachesMode(outputCache, callCache, sessionKey, payload, false)
}
