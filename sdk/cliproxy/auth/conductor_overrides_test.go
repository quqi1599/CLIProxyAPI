...
func TestManager_GPTLargeToolResponses_CapsCodexFallbackAtSixTotalAttempts(t *testing.T) {
	const model = "gpt-5.5"
	testCases := []struct {
		name      string
	...
	}{
		{
			...
			wantCalls: []string{"aa-codex-1", "ba-codex-3", "ca-codex-4", "da-codex-5", "ea-codex-6", "fa-codex-7"},
		},
		{
			name: "execute_count",
			invoke: func(m *Manager, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) error {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if errInvoke := tc.invoke(manager, request, opts); errInvoke == nil {
				t.Fatalf("expected guarded large tool request to fail after six attempts")
			}
...
func newGPTLargeToolResponsesFallbackManager(t *testing.T, model string, executor *authFallbackExecutor) (*Manager, *authFallbackExecutor) {
	...
	auths := []*Auth{
		...
		{ID: "ea-codex-6", Provider: "codex", Attributes: map[string]string{"routing_group": "group-e"}},
		{ID: "fa-codex-7", Provider: "codex", Attributes: map[string]string{"routing_group": "group-f"}},
	}
...
func TestIsInvalidGrantError_DoesNotMatchEchoedRequestText(t *testing.T) {
	errRequest := &Error{
		Code:       "invalid_request_error",
		HTTPStatus: http.StatusBadRequest,
		Message:    "request field must not contain the literal invalid_grant value",
	}
	if isInvalidGrantError(errRequest) {
		t.Fatal("plain request text containing invalid_grant must not be treated as an OAuth grant failure")
	}

	errStructured := &Error{
		Code:       "invalid_request_error",
		HTTPStatus: http.StatusBadRequest,
		Message:    `upstream response: {"error": "invalid_grant", "error_description": "Bad Request"}`,
	}
	if !isInvalidGrantError(errStructured) {
		t.Fatal("structured invalid_grant OAuth error was not detected")
	}
}
...
func TestRequestEntityTooLargeDoesNotRetryOrFallback(t *testing.T) {
	err := &Error{Code: "request_too_large", HTTPStatus: http.StatusRequestEntityTooLarge, Message: "request body too large"}
	if !isRequestInvalidError(err) {
...
func TestResultErrorFromCauseClassifiesLegacy413AsRequestScoped(t *testing.T) {
	resultErr := resultErrorFromCause(codedStatusError{status: http.StatusRequestEntityTooLarge, code: "request_too_large", msg: "request body too large"})
	if resultErr == nil {
		t.Fatal("resultErrorFromCause() returned nil")
	}
	if resultErr.Kind != string(failurecontract.RequestTooLarge) || resultErr.Scope != string(failurecontract.ScopeRequest) || resultErr.Retryable {
