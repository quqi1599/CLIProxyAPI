...
func buildErrorEventPayload(result Result, authSnapshot *Auth) ([]byte, bool) {
	...
	event := errorEvent{
		...
		AuthStatus: buildErrorEventAuthStatus(result, authSnapshot),
	}
	if result.Error != nil {
...
func buildErrorEventAuthStatus(result Result, authSnapshot *Auth) errorEventAuthStatus {
	...
	if result.Error != nil && model != "" && (status.Model == nil || !status.Model.Unavailable) {
		...
		if statusCode == 0 || statusCode == 429 || statusCode >= 500 {
			quota := (*errorEventQuotaStatus)(nil)
			if statusCode == 429 {
				quota = &errorEventQuotaStatus{Exceeded: true, Reason: "quota"}
			}
			status.Status = StatusError
			status.StatusMessage = strings.TrimSpace(result.Error.Message)
			status.Unavailable = true
			if status.Quota == nil {
				status.Quota = quota
			}
			status.Model = &errorEventModelStatus{
				Name:          model,
				Status:        StatusError,
				StatusMessage: strings.TrimSpace(result.Error.Message),
				Unavailable:   true,
				Quota:         quota,
			}
		}
