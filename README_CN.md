...
不同渠道。普通请求每轮最多尝试 8 个不同渠道并完整执行 2 轮；第 3 轮只重试
第 2 轮出现 `503` 或网络错误的渠道，或用于恢复 `auth_unavailable`。
大工具历史的 `/v1/responses` 请求仍限制为总计 6 次尝试。连续 5 次普通上游
`5xx`（协议级故障为 3 次），或 30 秒内至少 10 个渠道样本且失败率达到 80%，
会按 30、60、120 秒逐级冷却。CPA 自己触发的首事件 deadline 只用于换路和
自适应慢档，不计入渠道熔断。半开状态一次只放行一个探测请求，连续成功 2 次后关闭熔断。
同一模型出现零可用路由时，最多 8 个请求可有界等待同一个单飞半开探测，不再全部立即返回 503。
同一路由身份和 BaseURL 下的多个 API key 按一个渠道统计。
